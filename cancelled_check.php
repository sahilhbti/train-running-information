...
<html>
<head>
	<title>Welcome to railway enquiry system(unofficial)-List of Cancelled Trains</title>
	<style>
	.top
	{
...
<body>
   ...
   <h3 style="border-style: dotted;font-size: 30px;text-align: center;padding-top: 7px;padding-bottom: 7px;"><i>List Of Cancelled Train</i></h3>
   <br>  
     
<?php 
	  // $GLOBALS['get_string'] =  "https://api.railwayapi.com/v2/route/train/".$_POST["train_number"]."/apikey/tr8ghfhp7p/";
	  $GLOBALS['get_string'] =  "https://api.railwayapi.com/v2/cancelled/date/".$_POST["train_date"]."/apikey/zka0st3l0n/";
       $jsonData = file_get_contents($get_string);
        $GLOBALS['json']  =  json_decode($jsonData,true);
        error_reporting(E_ERROR | E_PARSE);?>
      <p style="margin-left: 100px;font-size: 20px;"><b> List of cancelled train starting on <?php  echo $_POST["train_date"]?></b></p>
         <table style="margin-left: 100px;margin-right: 130px;">
 		  ...
 		  <?php foreach ($json[trains] as $character) : ?>
            ...
            <td> <?php echo $character['source'][name]; ?></td>
            <td> <?php echo $character['dest'][name];?></td>
        </tr>
		<?php endforeach; ?>
