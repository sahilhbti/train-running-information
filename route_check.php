...
<body>
   ...
   <h3 style="border-style: dotted;font-size: 30px;text-align: center;padding-top: 7px;padding-bottom: 7px;"><i>Train Route</i></h3>
   <br>
  
   <table id="myTable" style="margin-left: 350px;">
   	<tbody>
		<tr>
			<th>TRAIN NAME</th>
			<th>TRAIN NUMBER</th>
	    ...
	    <td> 
	    	<?php
	    	$GLOBALS['get_string'] = "https://api.railwayapi.com/v2/route/train/".$_POST["train_number"]."/apikey/zka0st3l0n/";
   	  $jsonData = file_get_contents($get_string);
        $GLOBALS['json']  =  json_decode($jsonData,true);
	    	error_reporting(E_ERROR | E_PARSE);
     $first = $json['train'][name];
     echo "$first";      
     ?>
   </td>
   <td>
   ...
   <td>
   	<?php 
   	    error_reporting(E_ERROR | E_PARSE);
        $first = $json['train'][days]['0'][runs].$json['train'][days]['1'][runs].$json['train'][days]['2'][runs].$json['train'][days]['3'][runs].$json['train'][days]['4'][runs].$json['train'][days]['5'][runs].$json['train'][days]['6'][runs];
        echo "$first";?>
        </td>
</tr>
	</tbody>
 	...
 	<tbody>
 		...
 		<?php 
 		  ...
 		  <?php foreach ($json[route] as $character) : ?>
            ...
            <td> <?php echo $character['station']['name'];?></td>
           <td> <?php echo $character[scharr]; ?> </td>
           <td> <?php echo $character[schdep]; ?> </td>
           <td> <?php echo $character[day]; ?> </td>
        </tr>
		<?php endforeach; ?>
 	</tbody>
 </table>
