<!DOCTYPE html>
<html>
<head>
	<title>Welcome to railway enquiry system(unofficial)-Cancelled Trains</title>
	<style>
	.top
	{
	background-image: url("https://thumbs.dreamstime.com/t/wood-metallic-texture-background-rustic-banner-blue-tones-51204566.jpg");
        	}
	.top img
	{
		float: left;
	}
	.header
	{
		 overflow: hidden;
    background-color: #333;
	}
	.header a:hover {
    color: red;
 
}
	.header a
	{
     float: left;
    display: block;
    color: #f2f2f2;
    text-align: center;
    padding: 14px 16px;
    text-decoration: none;

	}
	h3
	{
		background-color: rgb(111, 140, 246) ;
	}
	.footer
	{

        background-image: url("http://www.modafinilsale.com/data/out/25/229332511-backgrounds-hd-wallpapers.jpg");
		border-radius: 8px;
	}
</style>
</head>
<body>
        <div class="top"> 
       <img src = "data:image/png;base64," style="width: 100px;height: 90px">
       <pre style="color: white;font-size: 30px;padding-top: 25px;border-style: dashed;padding-bottom: 10px;font-family: Andika";background-image: url("https://thumbs.dreamstime.com/twood-metallic-texture-background-rustic-banner-blue-tones-51204566.jpg");>                                               <u><b><i> INDIAN RAILWAY TRAIN ENQUIRY</i></b> </u> </pre>
   </div>
      <div class="header">
   	      <a href = "live_train.php">LIVE TRAIN </a>
      <a href = "live_station.html">LIVE STATION</a>
      <a href = "route.php">ROUTE</a>
      <a href = "pnr_check.html">PNR STATUS</a>
      <a href = "seat_availability.html">SEAT AVAILABILITY</a>
      <a href = "fare.html">FARE</a>
      <a href = "cancelled.php">CANCELLED TRAIN</a>
      <a href = "rescheduled.php">RESCHECULED TRAIN</a>
      <a href = "https://www.irctc.co.in/nget/">TICKET BOOKING</a>
   </div>
    <marquee style = "margin-top: 10px;font-size: 20px;color: red;" ><b>This website is only made for educational purpose only.It uses RailwayAPI api for informaion.There are limited number of queries we can perform.API need to be updated on 20-09-2018.<b></marquee>
   <h3 style="border-style: dotted;font-size: 30px;text-align: center;padding-top: 7px;padding-bottom: 7px;margin-top: 3px;"><i>Cancelled Train</i></h3>
   <br>
   <form style="background-color: rgb(183, 208, 208);" action="cancelled_check.php" method="post">
   	<fieldset>
   		<legend style="color: blue;"><b>Cancelled Train Information</b></legend>
   <p  style="margin-left: 100px;font-size: 20px;"> <b>TRAIN STARTING DATE</b></p>
   <hr>
   <script >
   	   var d = new Date();
   	   var present_date = d.getDate();
   	 document.getElementsByTag("select")[0].innerHTML=present_date;

   </script>
   <br>
  <select name="train_date" style="margin-left: 100px;font-size: 18px;padding-left: 10px;padding-right: 10px;padding-bottom: 5px;padding-top: 5px;">
  <option value="<?php date_default_timezone_set('Asia/Kolkata'); echo date("d-m-Y",strtotime("1 days"));?>"><?php date_default_timezone_set('Asia/Kolkata'); echo date("d-m-Y",strtotime("1 days"));?></option>
  <option value="<?php date_default_timezone_set('Asia/Kolkata');  echo date("d-m-Y",strtotime("0 days"));?>"><?php date_default_timezone_set('Asia/Kolkata');  echo date("d-m-Y",strtotime("0 days"));?></option>            
  <option value="<?php date_default_timezone_set('Asia/Kolkata');  echo date("d-m-Y",strtotime("-1 days"));?>"><?php date_default_timezone_set('Asia/Kolkata');  echo date("d-m-Y",strtotime("-1 days"));?></option>
  <option value="<?php date_default_timezone_set('Asia/Kolkata');  echo date("d-m-Y",strtotime("-2 days"));?>"><?php date_default_timezone_set('Asia/Kolkata');  echo date("d-m-Y",strtotime("-2 days"));?></option>
  <option value="<?php date_default_timezone_set('Asia/Kolkata'); echo date("d-m-Y",strtotime("-3 days"));?>"><?php date_default_timezone_set('Asia/Kolkata'); echo date("d-m-Y",strtotime("-3 days"));?></option>
</select>
    
   <input style="font-size: 20px;margin-left: 100px;" type="submit" name="submit">
   </fieldset>
</form>
<br>
 <div class="footer" style="padding-top: 10px;padding-bottom: 10px;">
 	   
        <p style = "text-align:center;color: white">INDIAN RAILWAY ENQUIRY SYSTEM</p>
         <p style=" text-align: center;color: white">MADE BY SAHIL SRIVASTAVA</p>
         <p style=" text-align: center;color: white">Email-id:sahilhbti@gmail.com</p>
         
          </div> 
</body>
</html>