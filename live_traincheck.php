...
<html>
<head>
	<title>Welcome to railway enquiry system(unofficial)-live train information</title>
	 <style>
	 table, th, td {
    border: 3px ;
    border-style: solid;
...
<body>
 <div class="top"> 
   ...
   <?php 
    $GLOBALS['get_string'] = "https://api.railwayapi.com/v2/live/train/".$_POST["hello"]."/date/".$_POST["get_date"]."/apikey/zka0st3l0n/";
   ...
   <table style="margin-left: 300px;">
   	...
   	<tr>
   		<th>Actual Arrival data</th>
   		<td><?php foreach ($json[route] as $character)  
 		{
 			if($character[station][code]==$_POST["cars"])
 			{
 				echo $character[actarr_date];
 			}
 		}  
 	?> </td>
   	</tr>	
 	...
 	?> 
   	...
   	<tr>
   		...
   		<td><?php foreach ($json[route] as $character)  
 		{
 			if($character[station][code]==$_POST["cars"])
 			{
 				echo $character[latemin]. " " ."minutes";
 			}
 		}  
 	?> </td>
   	</tr>
 	...
 	?> </td>
   	...
   	<tr>
   		<th>Last Location</th>
   		<td><?php  echo $json[position]?></td>
   	</tr>
   <tr>
     <th>Next Stoppage Station</th>
     <td><?php foreach ($json[route] as $character)  
    {
      if($character[has_arrived]==false)
      {
        echo $character[station][name];
        break;
      }
    }  
  ?>  </td>
