<!DOCTYPE html>
<html>
<head>
	<title>Welcome to railway enquiry system(unofficial)-Fare </title>
	<style >
	.top
	{
	background-image: url("https://thumbs.dreamstime.com/t/wood-metallic-texture-background-rustic-banner-blue-tones-51204566.jpg");
        	}
	.top img
	{
		float: left;
	}
	.header
	{
		 overflow: hidden;
    background-color: #333;
	}
	.header a:hover {
    color: red;
 
}
button:hover
{
	background-color: rgb(204, 204, 185);
}
	.header a
	{
     float: left;
    display: block;
    color: #f2f2f2;
    text-align: center;
    padding: 14px 16px;
    text-decoration: none;

	}
	h3
	{
		background-color: rgb(111, 140, 246) ;
	}
	.footer
 {
     background-image: url("http://www.modafinilsale.com/data/out/25/229332511-backgrounds-hd-wallpapers.jpg");
		border-radius: 8px;	
 }
 
</style>
</head>
<body>
          <div class="top"> 
       <img src = "data:image/png;base64," style="width: 100px;height: 90px">
       <pre style="color: white;font-size: 30px;padding-top: 25px;border-style: dashed;padding-bottom: 10px;font-family: Andika";background-image: url("https://thumbs.dreamstime.com/twood-metallic-texture-background-rustic-banner-blue-tones-51204566.jpg");>                                               <u><b><i> INDIAN RAILWAY TRAIN ENQUIRY</i></b> </u> </pre>
   </div>
      <div class="header">
   	      <a href = "live_train.php">LIVE TRAIN </a>
      <a href = "live_station.html">LIVE STATION</a>
      <a href = "route.php">ROUTE</a>
      <a href = "pnr_check.html">PNR STATUS</a>
      <a href = "seat_availability.html">SEAT AVAILABILITY</a>
      <a href = "fare.html">FARE</a>
      <a href = "cancelled.php">CANCELLED TRAIN</a>
      <a href = "rescheduled.php">RESCHECULED TRAIN</a>
      <a href = "https://www.irctc.co.in/nget/">TICKET BOOKING</a>
   </div>
   <h3 style="border-style: dotted;font-size: 30px;text-align: center;padding-top: 7px;padding-bottom: 7px;"><i>Train Fare Information</i></h3>
   
   <?php 
   	$GLOBALS['get_string'] = "https://api.railwayapi.com/v2/fare/train/".$_POST["train_number"]."/source/".$_POST["source_stn"]."/dest/".$_POST["dest_stn"]."/age/".$_POST["Age"]."/pref/".$_POST["class_name"]."/quota/".$_POST["quota_name"]."/date/".$_POST["journey_date"]."/apikey/zka0st3l0n/";
   	  $jsonData = file_get_contents($get_string);
        $GLOBALS['json']  =  json_decode($jsonData,true);
	    	error_reporting(E_ERROR | E_PARSE);
 ?>
   <br>
      <p style="padding-left: 200px;font-size: 20px;background-color: rgb(198, 232, 234);padding-top: 10px;padding-bottom: 10px;">1. Train Name  </p>
      <p style="padding-left: 200px; font-size: 19px;"><b> <?php echo $json['train']['name'] ?> </b></p>
      <hr>
      <p style="padding-left: 200px;font-size: 20px; background-color: rgb(198, 232, 234);padding-top: 10px;padding-bottom: 10px;">2. Train Number </p>
      <p style="padding-left: 200px; font-size: 19px;"> <b> <?php echo $json['train']['number']; ?></b> </p>
      <hr>
      <p style="padding-left: 200px;font-size: 20px;background-color: rgb(198, 232, 234);padding-top: 10px;padding-bottom: 10px;">3.  From Station: </p>
      <p style="padding-left: 200px; font-size: 19px;"><b> <?php echo $json['from_station']['name']; ?></b></p>
      <hr>
      <p style="padding-left: 200px;font-size: 20px;background-color: rgb(198, 232, 234);padding-top: 10px;padding-bottom: 10px;">4.  To Station: </p>
      <p style="padding-left: 200px; font-size: 19px;"> <b><?php echo $json['to_station']['name'];?></b></p>
      <hr>
      <p style="padding-left: 200px;font-size: 20px;background-color: rgb(198, 232, 234);padding-top: 10px;padding-bottom: 10px;">5.  Quota: </p>
      <p style="padding-left: 200px; font-size: 19px;"><b><?php echo $json['quota']['name'];?></b></p>
      <hr>
      <p style="padding-left: 200px;font-size: 20px;background-color: rgb(198, 232, 234);padding-top: 10px;padding-bottom: 10px;">6. Running days: </p>
      <p style="padding-left: 200px; font-size: 19px;"><b> <?php $first = $json['train'][days]['0'][runs].$json['train'][days]['1'][runs].$json['train'][days]['2'][runs].$json['train'][days]['3'][runs].$json['train'][days]['4'][runs].$json['train'][days]['5'][runs].$json['train'][days]['6'][runs];
        echo "$first";?></b></p>
        <hr>
      <p style="padding-left: 200px;font-size: 20px;background-color: rgb(198, 232, 234);padding-top: 10px;padding-bottom: 10px;">7. Fare: </p>
      <p style="padding-left: 200px; font-size: 19px;"><b><?php echo $json['fare'] ?> </b> </p>
      <div class="footer">
  	<p style = "text-align:center;color: white;padding-top: 20px;">INDIAN RAILWAY ENQUIRY SYSTEM</p>
         <p style=" text-align: center;color: white";>MADE BY SAHIL SRIVASTAVA</p>
         <p style=" text-align: center;color: white;padding-bottom: 20px;">Email-id:sahilhbti@gmail.com</p>
  </div>

</body>
</html>