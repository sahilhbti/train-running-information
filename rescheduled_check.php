...
<html>
<head>
  <title>Welcome to railway enquiry system(unofficial)-List of rescheduled train</title>
  <style>
  .top
  {
...
<body>
   ...
   <h3 style="border-style: dotted;font-size: 30px;text-align: center;padding-top: 7px;padding-bottom: 7px;"><i>List Of Rescheduled Train</i></h3>
   <br>  
     
<?php 
    // $GLOBALS['get_string'] =  "https://api.railwayapi.com/v2/route/train/".$_POST["train_number"]."/apikey/tr8ghfhp7p/";
    $GLOBALS['get_string'] =  "https://api.railwayapi.com/v2/rescheduled/date/".$_POST["train_date"]."/apikey/zka0st3l0n/";
       $jsonData = file_get_contents($get_string);
        $GLOBALS['json']  =  json_decode($jsonData,true);
        error_reporting(E_ERROR | E_PARSE);
        ?>
        
      <p style="margin-left: 100px;font-size: 20px;"><b> List of rescheduled train starting on <?php  echo $_POST["train_date"]?></b></p>
         <table style="margin-left: 100px;margin-right: 130px;">
          <tbody>
          <tr>
            <th>TRAIN NAME</th>
                <th>TRAIN NUMBER</th>
          ...
          <?php 
      error_reporting(E_ERROR | E_PARSE);?>
      <?php foreach ($json[trains] as $character) : ?>
        <tr>
            <td> <?php echo $character[name]; ?> </td>
            <td> <?php echo $character[number]; ?></td>
            <td> <?php echo $character[rescheduled_time]; ?></td>
            <td> <?php echo $character[rescheduled_date]; ?></td>
            <td> <?php echo $character[time_diff];?></td>
        </tr>
    <?php endforeach; ?>
